    ...
    if same_team_selected:
        ...
        if st.button("🎲 Run 10,000 Monte Carlo Simulations", use_container_width=True):
            with st.spinner("Processing anchored Monte Carlo game iterations..."):
                ...
                # Fixed Offense vs Defense Interaction with proper scaling coefficient (0.20)
                # Team A offense combined with Team B defense (where negative defense is elite)
                matchup_diff_a = (p_a["offense"] + p_b["defense"]) + hfa
                matchup_diff_b = (p_b["offense"] + p_a["defense"]) - hfa

                score_a_mean = league_avg_scoring + (matchup_diff_a * 0.20)
                score_b_mean = league_avg_scoring + (matchup_diff_b * 0.20)

                sim_scores_a = np.clip(np.random.normal(loc=max(3, score_a_mean), scale=5.5, size=n_sims), 0, 56)
                sim_scores_b = np.clip(np.random.normal(loc=max(3, score_b_mean), scale=5.5, size=n_sims), 0, 56)
                ...
                sim_total_yds_a = np.clip(np.random.normal(loc=320 + (sim_scores_a * 3.5), scale=30, size=n_sims), 150, 600)
                sim_total_yds_b = np.clip(np.random.normal(loc=320 + (sim_scores_b * 3.5), scale=30, size=n_sims), 150, 600)
